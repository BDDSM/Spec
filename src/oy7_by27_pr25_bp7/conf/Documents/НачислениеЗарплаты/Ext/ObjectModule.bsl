...
Процедура РассчитатьУдержания(НаборУдержания, Запрос)
	Запрос.Текст =
		...
		|	втУдержания.ВидРасчета КАК ВидРасчета,
		|	ЕСТЬNULL(БазаДополнительныеНачисления.ДнейБаза, 0) КАК ДнейПрогула
		|ИЗ
		|	втУдержания КАК втУдержания
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.Удержания.БазаДополнительныеНачисления(&Измерения, &Измерения, , Регистратор = &Регистратор) КАК БазаДополнительныеНачисления
		|		ПО втУдержания.НомерСтроки = БазаДополнительныеНачисления.НомерСтроки";
