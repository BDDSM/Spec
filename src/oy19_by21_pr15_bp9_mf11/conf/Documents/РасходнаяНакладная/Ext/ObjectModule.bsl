...
#Область ОПЕРАТИВНЫЙ_УЧЕТ
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		Если НЕ ВыборкаНоменклатура.ЭтоТовар Тогда
			ДобавитьДвижениеПродажи(ВыборкаНоменклатура.Номенклатура, ВыборкаНоменклатура.Номенклатура, ВыборкаНоменклатура.Количество, ВыборкаНоменклатура.Сумма, 0);
			
			СуммаУслуг = СуммаУслуг + ВыборкаНоменклатура.Сумма;
		...
		Если ОсталосьСписать <= ОстаткиТовара.Остаток Тогда
			...
			ДобавитьДвижениеПродажи(ВыборкаНоменклатура.Номенклатура, ВыборкаНоменклатура.Номенклатура, ВыборкаНоменклатура.Количество, ВыборкаНоменклатура.Сумма, Себестоимость);
		...
		Иначе
			Если ОстаткиТовара.Остаток > 0 Тогда
				...
				ДобавитьДвижениеПродажи(ВыборкаНоменклатура.Номенклатура, ВыборкаНоменклатура.Номенклатура, ОстаткиТовара.Остаток, СуммаПродажи, Себестоимость);
...
#Область БУХГАЛТЕРСКИЙ_УЧЕТ
	Если ЗначениеЗаполнено(Валюта) Тогда
		Отбор = Новый Структура("Валюта, Контрагент", Валюта, Контрагент);
		АктуальныйКурс = РегистрыСведений.КурсыВалют.ПолучитьПоследнее().Курс;
		Если НЕ ЗначениеЗаполнено(АктуальныйКурс) Тогда
			Отказ = Истина;
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не задан курс валюты для контрагента";
			Сообщение.Сообщить();
		...
		ЭлементБлокировки.УстановитьЗначение("Субконто2", Валюта);
		Блокировка.Заблокировать();
		...
		Проводка.СуммаУЕ = СуммаУслуг;
		Проводка.Сумма   = СуммаУслуг * АктуальныйКурс;
		Проводка.СубконтоДт[ВидСубконтоКонтрагент] = Контрагент;
		Проводка.СубконтоДт[ВидСубконтоВалюта]     = Валюта;
		Проводка.СубконтоДт[ВидСубконтоДокумент]   = Ссылка;
		
		Если Курс <> АктуальныйКурс Тогда
			ВыборкаДокументов = Документы.УстановкаКурсов.Выбрать(НачалоМесяца(Дата), КонецМесяца(Дата));
			Если ВыборкаДокументов.Следующий() Тогда
				ДокОбъект = ВыборкаДокументов.ПолучитьОбъект();
				
				НаборЗаписей = ДокОбъект.Движения.Проводки;
				НаборЗаписей.Прочитать();
				
				Для Каждого Запись Из НаборЗаписей Цикл
					Если Запись.СубконтоДт.Документ = Ссылка ИЛИ Запись.СубконтоКт.Документ = Ссылка Тогда
