...
Процедура РассчитатьДополнительныеНачисления(НаборДополнительныеНачисления, Запрос)
	Запрос.Текст =
		...
		|	ЕСТЬNULL(БазаОсновныеНачисления.ОтработаноДнейБаза, 0) КАК ОтработаноДнейБаза,
		|	ЕСТЬNULL(СведенияОСотрудникахСрезПоследних.ЛимитНаЛекарства, 0) КАК ЛимитНаЛекарства
		|ИЗ
		...
		|		ПО ДополнительныеНачисления.Сотрудник = СведенияОСотрудникахСрезПоследних.Сотрудник
		|			И ДополнительныеНачисления.Подразделение = СведенияОСотрудникахСрезПоследних.Подразделение
		|ГДЕ
